...
uint32_t int_ram_window_offsets[NUM_LPC_MAPPERS];

bool int_ram_memread_handler(uint32_t addr, uint8_t *data, uint8_t mapper_id) {
	uint32_t int_ram_offset = int_ram_window_offsets[mapper_id];
	volatile uint8_t *int_mem = (uint8_t*)0;
	uint32_t offset = int_ram_offset + (addr - lpc_mappers[mapper_id].base_addr);
...
uint8_t int_ram_handler_control_peek(uint8_t cmd, uint8_t data) {
	...
	case INT_RAM_CMD_WIN_OFFSET:
		current_long_val = int_ram_window_offsets[current_mapper_id];
		break;
	}
...
uint8_t int_ram_handler_control_set(uint8_t cmd, uint8_t data) {
	switch(cmd) {
	case INT_RAM_CMD_WIN_OFFSET:
		int_ram_window_offsets[current_mapper_id] = current_long_val;
		break;
	}
